...
>
> 配套文档：`readme.md`（项目做了什么、解决了什么问题，中英德三语）、`todo.md`（附录＝接口/配置/代码地图/56 项缺陷；
> 下方＝各轮次开发日志，可完整复盘整个过程）。
...
> 但我把代价都记在文档里了——比如所有服务共用一个 PostgreSQL 实例，这其实更接近**分布式单体**（见 `todo.md` 附录 §10 限制）。
...
> 而且**每一个踩过的坑都写在 `todo.md` 附录的 56 条缺陷表里**——包括根因和我改错了什么。
