...
pkill -9 -f smart-logistocs-platform || true
...
> 避免历史上「子目录 Dockerfile 无法访问根 pom.xml」的上下文越界问题。
...
│       └── security/VpnSecurityConfig.java                     # M4 VPN JWT
└── services/
...
| JWT 密钥 | `aslp.jwt.secret`（默认值仅供开发） | 环境变量 `ASLP_JWT_SECRET` 注入 |
...
| 新接口返回 404 / 服务启动报 `Failed to start bean 'webServerStartStop'` | **端口被上一轮遗留的旧进程占用**。执行 `pkill -9 -f smart-logistocs-platform` 后重启 |
