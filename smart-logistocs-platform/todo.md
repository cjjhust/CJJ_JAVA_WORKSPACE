...
| 17 | 新接口 404 / 服务启动报 `webServerStartStop` 失败 | 上一轮遗留的旧 jar 进程仍占用 8080-8085 端口 | 文档补充 `pkill -9 -f smart-logistocs-platform` 前置步骤；冒烟脚本启动前检查 |
...
- [ ] **P0-3 全量容器验证**：`docker compose up -d --build` 六服务全部 healthy
- [ ] **P0-4 Flyway**：替代 `ddl-auto: update`，切换 `validate`
...
- [ ] **P0-3 全量容器验证**：`docker compose up -d --build` 六个服务全部 `healthy`，并跑通 `scripts/smoke-test.sh`
- [ ] **P0-4 引入 Flyway**：`order-service` / `inventory-service` 统一用 `flyway-core` 管理版本化迁移，`ddl-auto: validate`
